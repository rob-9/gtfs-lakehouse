...
from gtfs_lakehouse.schedules import (
    parse_archive,
    enrich,
    seconds,
    service_instant,
...
def test_fixture_schedule_resolves_trip_and_cancellation():
    ...
    events, errors = normalize_feed(
        realtime(), agency_id="demo", feed_id="f", ingested_at=1
    )
    assert not errors
...
def test_gtfs_clock_preserves_hours_beyond_midnight():
    ...
    for day in (date(2026, 3, 8), date(2026, 11, 1)):
        assert (
            service_instant(day, "25:00:00", "America/Los_Angeles")
            - service_instant(day, "01:00:00", "America/Los_Angeles")
            == 86400
        )
...
def test_unknown_trip_is_preserved_with_reason():
    ...
    result = enrich(
        events[0].to_dict() | {"trip_id": "unknown"}, parse_archive(static_zip()), "v"
    )
