...
class Outbox:
    ...
    def __init__(self, path):
        ...
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS pending (id INTEGER PRIMARY KEY CHECK(id=1), metadata TEXT, body BLOB)"
        )
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS state (id INTEGER PRIMARY KEY CHECK(id=1), etag TEXT, modified TEXT)"
        )
    ...
    def stage(self, snapshot):
        ...
        with self.db:
            self.db.execute(
                "INSERT INTO pending VALUES (1, ?, ?)", (json.dumps(metadata), body)
            )

    def pending(self):
        row = self.db.execute(
            "SELECT metadata, body FROM pending WHERE id=1"
        ).fetchone()
...
class Publisher:
        ...
        self.producer = Producer(
            {
                ...
                "transactional.id": f"gtfs-poller-{identity}",
    ...
    def publish(self, snapshot):
        ...
        try:
            ...
            for event in events:
                event_key = canonical_json(
                    [event.agency_id, event.feed_id, event.entity_type, event.entity_id]
                )
                self.producer.produce(
                    "gtfs.normalized.events",
                    key=event_key,
                    value=encode_event(event.to_dict()),
                    on_delivery=delivered,
                )
            ...
            while not stop.is_set():
                ...
                try:
                    ...
                    if once:
                        raise
                    failures += 1
                    delay = min(300, interval * 2 ** min(failures, 8)) * random.uniform(
                        0.8, 1.2
                    )
                    ...
                    print(
                        json.dumps(
                            {"feed": entry["feed_id"], "error": type(exc).__name__}
                        ),
                        flush=True,
                    )
                    publisher = None
