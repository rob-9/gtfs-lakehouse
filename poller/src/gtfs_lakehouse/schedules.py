...
def parse_archive(body):
    ...
    with ZipFile(BytesIO(body)) as archive:
        ...
        for name in REQUIRED.keys() | {"calendar", "calendar_dates"}:
            ...
            reader = csv.DictReader(
                StringIO(archive.read(f"{name}.txt").decode("utf-8-sig"))
            )
    ...
    for stop in result["stops"]:
        if (
            not -90 <= float(stop["stop_lat"]) <= 90
            or not -180 <= float(stop["stop_lon"]) <= 180
        ):
...
def active(schedule, service, day):
    ...
    exceptions = [
        row
        for row in schedule.get("calendar_dates", [])
        if row["service_id"] == service and row["date"] == compact
    ]
    if exceptions:
    ...
    return any(
        ...
        and row["start_date"] <= compact <= row["end_date"]
        and row[weekday] == "1"
        for row in schedule.get("calendar", [])
    )
...
def enrich(event, schedule, version):
    ...
    trip = next(
        (row for row in schedule["trips"] if row["trip_id"] == event["trip_id"]), None
    )
    ...
    return result | {
        ...
        "direction_id": int(trip.get("direction_id") or -1),
